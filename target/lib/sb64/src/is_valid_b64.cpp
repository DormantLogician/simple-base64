...
namespace sb64
{
    bool is_valid_b64(const std::function<std::optional<unsigned char>()>& reader)
    {
        const std::function padded_correctly{[](const std::vector<unsigned char>& input) -> bool
        {
            ...
            const long long expected_padds{detail::constant::decoder_word_size - size_without_padds};
        ...
        const std::function has_only_b64_letters{[](const std::vector<unsigned char>& input) -> bool
        {
            for (auto character{input.cbegin()}; character < input.cend(); ++character)
            {
                ...
                if (!exists_in_alphabet && (*character != detail::constant::padd_char))
                {
                    return false;
                };
            };
        ...
        {
            const bool is_correct_size{output.size() == detail::constant::decoder_word_size};
        ...
        detail::utility::write_stream(detail::constant::decoder_word_size, reader, valid);
        return is_b64;
    };
};
